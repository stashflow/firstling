...
alter table clients add column if not exists setup_email_sent_at timestamp with time zone;
alter table clients add column if not exists ready_email_sent_at timestamp with time zone;
alter table clients add column if not exists last_portal_login_at timestamp with time zone;
...
create table if not exists overage_billing_runs (
  id uuid primary key default gen_random_uuid(),
  client_id uuid not null references clients(id) on delete cascade,
  month date not null,
  overage_minutes integer not null default 0,
...
create index if not exists clients_clerk_user_idx on clients (clerk_user_id);
create index if not exists clients_notification_email_idx on clients (lower(notification_email));
...
create index if not exists overage_billing_runs_month_idx on overage_billing_runs (month desc);
